...
type
  TdmCustomModule = class (TDataModule, IConnection,
    ...
    procedure cdsMasterBeforeOpen(DataSet: TDataSet);
  ...
  protected
    ...
    procedure InitFields(ADataSet : TDataSet); overload;
    procedure InitFields; overload; virtual;
...
uses
  Variants, Forms, Controls, StrUtils, Dialogs, DBCommon,
...
procedure TdmCustomModule.cdsMasterBeforeOpen(DataSet: TDataSet);
begin
  SourceDataSet.Active := False;
  cdsMaster.IndexName := '';
end;
...
procedure TdmCustomModule.cdsMasterAfterDelete(DataSet: TDataSet);
begin
  PostUpdates;
  ExecuteDetails;
end;
...
procedure TdmCustomModule.cdsMasterAfterOpen(DataSet: TDataSet);
begin
  InitFields;
  ExecuteDetails;
end;
...
procedure TdmCustomModule.cdsMasterAfterPost(DataSet: TDataSet);
begin
  inherited;
  PostUpdates;
end;
...
procedure TdmCustomModule.cdsMasterAfterScroll(DataSet: TDataSet);
begin
  if DataSet.RecNo > RecordCount then
    RecordCount := DataSet.RecNo;
  ExecuteDetails;
end;
...
begin
  ...
  begin
    ...
    begin
      (SourceDataSet as IProviderSupport).PSSetCommandText(ACommandText);
...
{ Calls InitField for every field in the master dataset. }

procedure TdmCustomModule.InitFields;
begin
  InitFields(DataSet);
end;
...
//  try
//    Refreshed := False;
//    DS.Connection := (Connection as IADOConnection).ADOConnection;
...
//      DS.CommandText := SQLSelect + SQLFrom + SQLWhere;
//      try
//        DS.Active := True;
  ...
  // check for required fields
    ...
    if B then
      try
        ...
        Abort;
        raise Exception.Create('foute invoer');
      end;
    Result := B;
  end
  else
...
{ Post all pending updates in cdsMaster to the database. }

procedure TdmCustomModule.PostUpdates;
begin
  PostUpdates(cdsMaster);
  UpdateDisplayValues;
end;
