...
unit ts.Connection.ADOConnectionAdapter;
...
type
  TADOConnectionAdapter = class(TCustomConnectionAdapter, IConnection, IMetaData)
  private
...
procedure TADOConnectionAdapter.AfterConstruction;
begin
  inherited;
...
procedure TADOConnectionAdapter.BeforeDestruction;
begin
  FConnection.Free;
  inherited;
end;
...
procedure TADOConnectionAdapter.SetConnected(const Value: Boolean);
begin
...
function TADOConnectionAdapter.GetConnection: TComponent;
begin
  Result := FConnection;
end;
...
function TADOConnectionAdapter.GetConnectionType: string;
begin
  Result := 'ADO';
end;
{$ENDREGION}
...
procedure TADOConnectionAdapter.AssignConnectionSettings;
var
  S: string;
begin
  inherited;
...
procedure TADOConnectionAdapter.AssignConnectionString(const AValue: string);
var
  B: Boolean;
  S: WideString;
begin
...
function TADOConnectionAdapter.CreateNativeDataSet: INativeDataSet;
begin
  Result := TNativeADODataSet.Create(Self);
end;

function TADOConnectionAdapter.Execute(const ACommandText: string): Boolean;
begin
  FConnection.Execute(ACommandText);
  Result := True;
end;

procedure TADOConnectionAdapter.GetFieldNames(AList: TStrings;
...
procedure TADOConnectionAdapter.GetSchemaNames(AList: TStrings);
var
  DS : TADODataSet;
begin
  AList.Clear;
...
procedure TADOConnectionAdapter.GetTableNames(AList: TStrings;
