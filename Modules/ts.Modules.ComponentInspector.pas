...
unit ts.Modules.ComponentInspector;
...
type
  TtsComponentInspector = class(TForm)
    pnlMain      : TPanel;
...
procedure InspectComponent(AComponent : TComponent);
var
  InspectorForm : TtsComponentInspector;
begin
  if Assigned(AComponent) then
  begin
    InspectorForm := TtsComponentInspector.Create(Application, AComponent);
    InspectorForm.Show;
  end
  else
...
begin
  ...
  begin
    InspectorForm := TtsComponentInspector.Create(Application, AObject);
    InspectorForm.Show;
  end
  else
    raise Exception.Create('No component Assigned');
...
begin
  ...
  begin
    InspectorForm := TtsComponentInspector.Create(Application, AComponents[0]);
...
constructor TtsComponentInspector.Create(AOwner: TComponent; AObject: TPersistent);
begin
...
procedure TtsComponentInspector.CMDialogKey(var Msg: TCMDialogKey);
begin
...
procedure TtsComponentInspector.CreatePropertyInspector;
begin
...
procedure TtsComponentInspector.AddComponentToInspector(
...
procedure TtsComponentInspector.FocusComponentInInspector(
  AComponent: TPersistent);
begin
...
procedure TtsComponentInspector.cbxInspectorChange(Sender: TObject);
begin
...
procedure TtsComponentInspector.FormActivate(Sender: TObject);
begin
  FInspector.UpdateItems;
end;

procedure TtsComponentInspector.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure TtsComponentInspector.FormResize(Sender: TObject);
begin
  FInspector.Splitter := FInspector.ClientWidth div 2;
end;

procedure TtsComponentInspector.FormShow(Sender: TObject);
begin
