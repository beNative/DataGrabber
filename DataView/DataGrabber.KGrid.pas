unit DataGrabber.KGrid;

interface

uses
  Windows, Messages, SysUtils, Variants, Classes, Graphics, Controls, Forms,
  Dialogs, DB, Generics.Collections,
...
type
  TfrmKGrid = class(TForm, IDataView, IDGDataView)
    grdMain: TKDBGrid;
    dscMain: TDataSource;
    procedure grdMainCellSpan(Sender: TObject; ACol, ARow: Integer;
      var Span: TKGridCellSpan);
...
procedure TfrmKGrid.grdMainCellSpan(Sender: TObject; ACol, ARow: Integer;
  var Span: TKGridCellSpan);
begin
//
end;
...
begin
  ...
  begin
    F := DataSet.FindField(TKDBGridCol(grdMain.Cols[ACol]).Name);
    if Assigned(F) then
    begin
    ...
    try
      ...
      try
        ...
        begin
          ...
          if AIncludeHeader then
          begin
            SL.Add(TKDBGridCol(grdMain.Cols[X]).Name);
          end;
          for Y := SR.Row1 to SR.Row2 do
          begin
            S := grdMain.Cells[X, Y];
      ...
      begin
        ...
        begin
          ...
          sTxt := sTxt + '|' + Format(sFmt, [TKDBGridCol(grdMain.Cols[X]).Name]);
        end;
        sTxt := sTxt + '|';
        sLine := sLine + '+';
